...
\begin{figure}[t]
\centering
\includegraphics[width=0.5\linewidth]{fig4_memory.png}
\caption{Measured KV-cache memory at 2048 tokens (Llama-2-7B). The packed 2-bit
value cache is 3.9$\times$ smaller than fp16 and 20\% smaller than KIVI's INT4.}
\label{fig:mem}
\end{figure}
...
measurements (Figure~\ref{fig:atscale}).

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig7_atscale_threeway.png}
\caption{Independent at-scale reproduction (dual-T4): cache-path perplexity,
measured KV memory, and decode throughput for fp16, KIVI, and B2 on Llama-2-7B.
B2 matches KIVI quality with less memory.}
\label{fig:atscale}
\end{figure}
